...
void GaugeMap::Initialize(std::vector<GaugeConfigSection *> *newGauges) {
	...
	// Resize the outer vector in the tree that contains all of the interior gauges
...
void GaugeMap::GaugeAverage(std::vector<GridNode> *nodes, std::vector<float> *currentValue, std::vector<float> *gaugeAvg) {
	size_t countGauges = gauges.size();
