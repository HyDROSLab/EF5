...
bool PETReader::Read(char *file, SUPPORTED_PET_TYPES type, std::vector<GridNode> *nodes, std::vector<float> *currentPET, float petConvert, bool isTemp, float jday, std::vector<float> *prevPET) {
	if (!strcmp(lastPETFile, file)) {
		...
		return true; // This is the same pet file that we read last time, we assume currentPET is still valid!
	}
	...
	// We have two options now... Either the pet grid & the basic grids are the same
	...
	if (g_DEM->IsSpatialMatch(petGrid)) {
		...
		GridLoc pt;
                for (size_t i = 0; i < nodes->size(); i++) {
                        GridNode *node = &(nodes->at(i));
                        if (petGrid->GetGridLoc(node->refLoc.x, node->refLoc.y, &pt) && petGrid->data[pt.y][pt.x] > 0.0) {
	...
	// See if this is a temperature grid and if so convert it into PET using Hamon (1961).
	if (isTemp) {
		for (size_t i = 0; i < nodes->size(); i++) {
                        GridNode *node = &(nodes->at(i));
				...
				//printf("(%f, %f) %f, %f, %f, %f, %f, %f\n", lat, lon, currentPET->at(i), e_s, delta, omega_s, H_t, E_t);
