...
public:
  ...
  bool SnowBalance(float jday, float stepHours, std::vector<float> *precip, std::vector<float> *temp, std::vector<float> *melt, std::vector<float> *swe);
...
private:
  ...
  void SnowBalanceInt(GridNode *node, Snow17GridNode *cNode, float stepHours, float jday, float precipIn, float tempIn, float *melt, float *swe);
