...
std::map<std::string, InundationCaliParamConfigSection *> g_inundationCaliParamConfigs[INUNDATION_QTY];

InundationCaliParamConfigSection::InundationCaliParamConfigSection(char *nameVal, INUNDATIONS inundationVal) {
	strcpy(name, nameVal);
	gauge = NULL;
...
CONFIG_SEC_RET InundationCaliParamConfigSection::ValidateSection() {
	...
	for (int i = 0; i < numParams; i++) {
		if (!paramsSet[i]) {
			ERROR_LOGF("Incomplete inundation parameter set! Parameter \"%s\" was not given a value.", inundationParamStrings[inundation][i]);
			return INVALID_RESULT;
		}
	}
...
bool InundationCaliParamConfigSection::IsDuplicate(char *name, INUNDATIONS inundationVal) {
        std::map<std::string, InundationCaliParamConfigSection *>::iterator itr = g_inundationCaliParamConfigs[inundationVal].find(name);
