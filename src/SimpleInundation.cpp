...
bool SimpleInundation::InitializeModel(std::vector<GridNode> *newNodes, std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
  ...
  for (size_t i = 0; i < numNodes; i++) {
    ...
    node->modelIndex = i;
		while (!channelNode->channelGridCell && channelNode->downStreamNode != INVALID_DOWNSTREAM_NODE) {
			channelNode = &nodes->at(channelNode->downStreamNode);
		}
		iNodes[i].params[PARAM_SI_ALPHA] = iNodes[channelNode->index].params[PARAM_SI_ALPHA];
		iNodes[i].params[PARAM_SI_BETA] = iNodes[channelNode->index].params[PARAM_SI_BETA];
...
bool SimpleInundation::Inundation(std::vector<float> *discharge, std::vector<float> *depth) {
  ...
  for (size_t i = 0; i < numNodes; i++) {
    ...
    InundationInt(node, cNode, discharge->at(cNode->channelIndex),  &(depth->at(i)));
  }
  
  return true;
}

void SimpleInundation::InundationInt(GridNode *node, InundationGridNode *cNode, float dischargeIn, float *depth) {

	//float area = powf(dischargeIn/cNode->params[PARAM_SI_ALPHA], 1.0/cNode->params[PARAM_SI_BETA]);
	//float z1 = 3.0, z2 = 3.0, b = 100.0;
	//float b= 50.0;
	float height = cNode->params[PARAM_SI_ALPHA]*powf(dischargeIn, cNode->params[PARAM_SI_BETA]);//area/b; //(-b + sqrtf(powf(b, 2.0) - 4.0 * area * (z1+z2))) / (2.0 * (z1+z2));
	//if (node->channelGridCell && dischargeIn != 0.0) {
	//	printf("Have Q %f, area %f, height %f, elev %f\n", dischargeIn, area, height, cNode->elevDiff);
	//}
...
void SimpleInundation::InitializeParameters(std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
  ...
  for (size_t i = 0; i < numNodes; i++) {
    ...
    memcpy(cNode->params, (*paramSettings)[node->gauge], sizeof(float)*PARAM_SI_QTY);
