 ...
 *array = (float**)malloc(nrows*sizeof(float *));
 ...
 if (*array == NULL)
 {
  ...
  for(i = 0; i < nrows; i++) 
  {
   ...
   if ((*array)[i] == NULL)
   {
    printf("ERROR in misc_functions.c/allocate2D at line 23: No Memory for array %d\n", i);
    exit(1);
    }
   }
...
float sumarray(float *array, int nrows, int ncols)
{
 ...
 if (nrows < 0 || ncols < 0)
 {
  printf("ERROR: sumarray: number of columns and/or number of rows must be positive integer. nrows = %d, ncols = %d\n", nrows, ncols);
  exit(1);
  }
 if (nrows == 1 || ncols == 1)
 {
