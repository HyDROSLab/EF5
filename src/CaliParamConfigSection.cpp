...
char *CaliParamConfigSection::GetName() {
	return name;
}

CONFIG_SEC_RET CaliParamConfigSection::ProcessKeyValue(char *name, char *value) {
	int numParams = numModelParams[model];
...
CONFIG_SEC_RET CaliParamConfigSection::ValidateSection() {
	...
	for (int i = 0; i < numParams; i++) {
		if (!paramsSet[i]) {
			ERROR_LOGF("Incomplete parameter set! Parameter \"%s\" was not given a value.", modelParamStrings[model][i]);
			return INVALID_RESULT;
		}
	}
...
bool CaliParamConfigSection::IsDuplicate(char *name, MODELS modelVal) {
        std::map<std::string, CaliParamConfigSection *>::iterator itr = g_caliParamConfigs[modelVal].find(name);
