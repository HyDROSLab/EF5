...
float GeographicProjection::GetLen(float x, float y, FLOW_DIR dir) {
	...
	switch (dir) {
		...
		case FLOW_SOUTHWEST:
		case FLOW_NORTHWEST:
			return sqrt(pow(metersSNPerCell * cos(TORADIANS(y)), 2.0) + pow(metersSNPerCell, 2.0));
		default:
...
void GeographicProjection::ReprojectPoint(float lon, float lat, float *x, float *y) {
...
void GeographicProjection::UnprojectPoint(float x, float y, float *lon, float *lat) {
