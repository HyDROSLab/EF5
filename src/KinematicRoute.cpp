...
bool KWRoute::InitializeModel(std::vector<GridNode> *newNodes, std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
...
void KWRoute::InitializeStates(TimeVar *beginTime, char *statePath, std::vector<float> *fastFlow, std::vector<float> *slowFlow) {
  DatedName timeStr;
  ...
  char buffer[255];
  for (int p = 0; p < STATE_KW_QTY; p++) {
    sprintf(buffer, "%s/kwr_%s_%s.tif", statePath, stateStrings[p], timeStr.GetName());
...
void KWRoute::SaveStates(TimeVar *currentTime, char *statePath, GridWriterFull *gridWriter) {
  DatedName timeStr;
  timeStr.SetNameStr("YYYYMMDD_HHUU");
  timeStr.ProcessNameLoose(NULL);
...
bool KWRoute::Route(float stepHours, std::vector<float> *fastFlow, std::vector<float> *slowFlow, std::vector<float> *discharge) {
  ...
  for (size_t i = 0; i < numNodes; i++) {
    ...
    cNode->states[STATE_KW_IR] = cNode->states[STATE_KW_IR] + cNode->incomingWater[KW_LAYER_INTERFLOW];
    cNode->incomingWater[KW_LAYER_INTERFLOW] = 0.0; // Zero here so we can save states
    cNode->incomingWater[KW_LAYER_FASTFLOW] = 0.0; // Zero here so we can save states
  }
...
void KWRoute::RouteInt(float stepSeconds, GridNode *node, KWGridNode *cNode, float fastFlow, float slowFlow ) {
  ...
  if (!cNode->channelGridCell) {
    ...
    if (cNode->incomingWaterOverland + cNode->states[STATE_KW_PQ] > 0.0) {
      backDiffq =pow((cNode->incomingWaterOverland + cNode->states[STATE_KW_PQ]) / 2.0, beta - 1.0);
    ...
    float rhs = A + alpha * pow(cNode->states[STATE_KW_PQ], beta) + stepSeconds * newInWater;
    for (int itr = 0; itr < 10; itr++) {
        float resError = (stepSeconds / node->horLen) * estq + alpha * pow(estq, beta) - rhs;
        ...
        float resErrorD1 = (stepSeconds / node->horLen) + alpha * beta * pow(estq, beta - 1.0);
    ...
    double interflowLeak = cNode->states[STATE_KW_IR] * cNode->params[PARAM_KINEMATIC_LEAKI];
    ...
    if (cNode->routeCNode[0][KW_LAYER_INTERFLOW]) {
      double interflowLeak0 = interflowLeak * cNode->routeAmount[0][KW_LAYER_INTERFLOW] * node->area / cNode->routeNode[0][KW_LAYER_INTERFLOW]->area;
      double leakAmount = interflowLeak0;
      /*if (cNode->routeNode[0][KW_LAYER_INTERFLOW]->channelGridCell) {
      printf(" 0 got %f ", cNode->routeCNode[0][KW_LAYER_INTERFLOW]->incomingWater[KW_LAYER_INTERFLOW]);
      }*/
    ...
    if (cNode->routeCNode[1][KW_LAYER_INTERFLOW]) {
      double interflowLeak1 = interflowLeak * cNode->routeAmount[1][KW_LAYER_INTERFLOW] * node->area / cNode->routeNode[1][KW_LAYER_INTERFLOW]->area;
      double leakAmount = interflowLeak1;
      //printf(" 1 got %f ", leakAmount);
      double *res = &(cNode->routeCNode[1][KW_LAYER_INTERFLOW]->incomingWater[KW_LAYER_INTERFLOW]);
      *res += leakAmount; // Make this an atomic add for parallelization
    }
    ...
    if (cNode->incomingWaterOverland + cNode->states[STATE_KW_PO] > 0.0) {
      backDiffq =pow((cNode->incomingWaterOverland + cNode->states[STATE_KW_PO]) / 2.0, beta - 1.0);
      if (!std::isfinite(backDiffq)) {
        backDiffq = 0.0;
      }
    }
    ...
    E = alpha * beta * backDiffq;
    float estq = (A + B + C)/(D + E); //cms/m
    float rhs = A + alpha * pow(cNode->states[STATE_KW_PO], beta) + stepSeconds * newInWater;
    for (int itr = 0; itr < 10; itr++) {
        float resError = (stepSeconds / node->horLen) * estq + alpha * pow(estq, beta) - rhs;
        if (!std::isfinite(resError)) {
                resError = 0.0;
    ...
    if (cNode->incomingWaterChannel + cNode->states[STATE_KW_PQ] > 0.0) {
      backDiffQ =pow((cNode->incomingWaterChannel + cNode->states[STATE_KW_PQ]) / 2.0, beta - 1.0);
      if (!std::isfinite(backDiffQ)) {
	backDiffQ = 0.0;
      }
    }
    ...
    rhs = A + alpha * pow(cNode->states[STATE_KW_PQ], beta) + stepSeconds * newq;
    for (int itr = 0; itr < 10; itr++) {
    	float resError = (stepSeconds / node->horLen) * estQ + alpha * pow(estQ, beta) - rhs;
        if (!std::isfinite(resError)) {
		resError = 0.0;
    ...
    printf("New water is %f (%f, %f) %f %f [%f %f %f %f %f] %f %f\n", newWater, cNode->incomingWaterChannel, cNode->states[STATE_KW_PQ], newq, cNode->incomingWaterOverland, A, B, C, D, E, alpha, 0.0);
    }*/
    cNode->states[STATE_KW_PQ] = newWater; //Update previous Q for further routing if "steps" > 1
    if (node->downStreamNode != INVALID_DOWNSTREAM_NODE) {
...
void KWRoute::InitializeParameters(std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
  ...
  size_t numNodes = nodes->size();
	...
	for (size_t i = 0; i < numNodes; i++) {
		...
		memcpy(cNode->params, (*paramSettings)[node->gauge], sizeof(float)*PARAM_KINEMATIC_QTY);
...
void KWRoute::InitializeRouting(float timeSeconds) {
  ...
  //This pass distributes parameters & calculates the time it takes for water to cross the grid cell.
	size_t numNodes = nodes->size();
	for (size_t i = 0; i < numNodes; i++) {
		GridNode *node = &nodes->at(i);
		KWGridNode *cNode = &(kwNodes[i]);
  ...
  for (size_t i = 0; i < numNodes; i++) {
    ...
    currentNode = node;
		previousNode = NULL;
    while (currentSeconds < timeSeconds && currentNode && !kwNodes[currentNode->modelIndex].channelGridCell) {
      if (currentNode) {
				previousSeconds = currentSeconds;
				previousNode = currentNode;
        currentSeconds += kwNodes[currentNode->modelIndex].nexTime[KW_LAYER_INTERFLOW];
        ...
        break; // We have effectively run out of nodes to transverse, this is done!
      }
    }
    
    cNode->routeNode[0][KW_LAYER_INTERFLOW] = currentNode;
		cNode->routeCNode[0][KW_LAYER_INTERFLOW] = (currentNode) ? &(kwNodes[currentNode->modelIndex]) : NULL;
		cNode->routeNode[1][KW_LAYER_INTERFLOW] = previousNode;
    cNode->routeCNode[1][KW_LAYER_INTERFLOW] = (previousNode) ? &(kwNodes[previousNode->modelIndex]) : NULL;
    if (currentNode && !kwNodes[currentNode->modelIndex].channelGridCell) {
    if ((currentSeconds - previousSeconds) > 0) {
			cNode->routeAmount[0][KW_LAYER_INTERFLOW] = (timeSeconds - previousSeconds) / (currentSeconds - previousSeconds);
      cNode->routeAmount[1][KW_LAYER_INTERFLOW] = 1.0 - cNode->routeAmount[0][KW_LAYER_INTERFLOW];
		}
    } else {
