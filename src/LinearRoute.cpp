...
bool LRRoute::InitializeModel(std::vector<GridNode> *newNodes, std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
...
void LRRoute::InitializeStates(TimeVar *beginTime, char *statePath, std::vector<float> *fastFlow, std::vector<float> *slowFlow) {
  
}
void LRRoute::SaveStates(TimeVar *currentTime, char *statePath, GridWriterFull *gridWriter) {
  
}

bool LRRoute::Route(float stepHours, std::vector<float> *fastFlow, std::vector<float> *slowFlow, std::vector<float> *discharge) {
...
void LRRoute::RouteInt(GridNode *node, LRGridNode *cNode, float fastFlow, float slowFlow) {
  
  if (!node->channelGridCell) {
    cNode->reservoirs[LR_LAYER_OVERLAND] += fastFlow;
  }
  
  double overlandLeak = cNode->reservoirs[LR_LAYER_OVERLAND] * cNode->params[PARAM_LINEAR_LEAKO];
	...
	double interflowLeak = cNode->reservoirs[LR_LAYER_INTERFLOW] * cNode->params[PARAM_LINEAR_LEAKI];
  ...
  if (cNode->reservoirs[LR_LAYER_INTERFLOW] < 0) {
	...
	if (cNode->routeCNode[0][LR_LAYER_OVERLAND]) {
		double overlandLeak0 = overlandLeak *  cNode->routeAmount[0][LR_LAYER_OVERLAND] * node->area / cNode->routeNode[0][LR_LAYER_OVERLAND]->area;
	...
	if (cNode->routeCNode[1][LR_LAYER_OVERLAND]) {
		double overlandLeak1 = overlandLeak *  cNode->routeAmount[1][LR_LAYER_OVERLAND] * node->area / cNode->routeNode[1][LR_LAYER_OVERLAND]->area;
    ...
    *res += leakAmount; // Make this an atomic add for parallelization
	...
	if (cNode->routeCNode[0][LR_LAYER_INTERFLOW]) {
		double interflowLeak0 = interflowLeak * cNode->routeAmount[0][LR_LAYER_INTERFLOW] * node->area / cNode->routeNode[0][LR_LAYER_INTERFLOW]->area;
    ...
    *res += leakAmount; // Make this an atomic add for parallelization
    ...
    double interflowLeak1 = interflowLeak * cNode->routeAmount[1][LR_LAYER_INTERFLOW] * node->area / cNode->routeNode[1][LR_LAYER_INTERFLOW]->area;
...
void LRRoute::InitializeParameters(std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
  ...
  size_t numNodes = nodes->size();
	...
	for (size_t i = 0; i < numNodes; i++) {
		...
		memcpy(cNode->params, (*paramSettings)[node->gauge], sizeof(float)*PARAM_LINEAR_QTY);
...
void LRRoute::InitializeRouting(float timeSeconds) {
  ...
  //This pass distributes parameters & calculates the time it takes for water to cross the grid cell.
	...
	for (size_t i = 0; i < numNodes; i++) {
		...
		float waterDepth = (cNode->reservoirs[LR_LAYER_OVERLAND] * cNode->params[PARAM_LINEAR_LEAKO] + cNode->reservoirs[LR_LAYER_INTERFLOW] * cNode->params[PARAM_LINEAR_LEAKI]) / 1000.0f;
		if (node->channelGridCell) {
    ...
    // Calculate the approximate speed of the water in meters per second (thus COEM is in meters per second)
    // Slope is meters vertical change over meters horizontal change so thus unitless
    float speed = pow(waterDepth, 0.66) * cNode->slopeSqrt;
    
    // We have different mannings roughness multipliers for overland & channel grid cells
		if (node->fac > cNode->params[PARAM_LINEAR_TH]) {
  ...
  for (size_t i = 0; i < numNodes; i++) {
		...
		cNode->routeCNode[0][LR_LAYER_OVERLAND] = (currentNode) ? &(lrNodes[currentNode->modelIndex]) : NULL;
		cNode->routeNode[1][LR_LAYER_OVERLAND] = previousNode;
		cNode->routeCNode[1][LR_LAYER_OVERLAND] = (previousNode) ? &(lrNodes[previousNode->modelIndex]) : NULL;
		if ((currentSeconds - previousSeconds) > 0) {
			cNode->routeAmount[0][LR_LAYER_OVERLAND] = (timeSeconds - previousSeconds) / (currentSeconds - previousSeconds);
			cNode->routeAmount[1][LR_LAYER_OVERLAND] = 1.0 - cNode->routeAmount[0][LR_LAYER_OVERLAND];
		}
    ...
    cNode->routeNode[0][LR_LAYER_INTERFLOW] = currentNode;
		cNode->routeCNode[0][LR_LAYER_INTERFLOW] = (currentNode) ? &(lrNodes[currentNode->modelIndex]) : NULL;
		cNode->routeNode[1][LR_LAYER_INTERFLOW] = previousNode;
    cNode->routeCNode[1][LR_LAYER_INTERFLOW] = (previousNode) ? &(lrNodes[previousNode->modelIndex]) : NULL;
    if ((currentSeconds - previousSeconds) > 0) {
			cNode->routeAmount[0][LR_LAYER_INTERFLOW] = (timeSeconds - previousSeconds) / (currentSeconds - previousSeconds);
      cNode->routeAmount[1][LR_LAYER_INTERFLOW] = 1.0 - cNode->routeAmount[0][LR_LAYER_INTERFLOW];
		}
	}
}
