...
char *EnsTaskConfigSection::GetName() {
        return name;
}

RUNSTYLE EnsTaskConfigSection::GetRunStyle() {
        return style;
}
...
CONFIG_SEC_RET EnsTaskConfigSection::ProcessKeyValue(char *name, char *value) {
	if (!strcasecmp(name, "style")) {
		...
		TOLOWER(value);
		std::map<std::string, TaskConfigSection *>::iterator itr = g_taskConfigs.find(value);
...
bool EnsTaskConfigSection::IsDuplicate(char *name) {
        std::map<std::string, EnsTaskConfigSection *>::iterator itr = g_ensTaskConfigs.find(name);
