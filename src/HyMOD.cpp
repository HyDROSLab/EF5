...
bool HyMOD::InitializeModel(std::vector<GridNode> *newNodes, std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
...
void HyMOD::SaveStates(TimeVar *currentTime, char *statePath, GridWriterFull *gridWriter) {

}

bool HyMOD::WaterBalance(float stepHours, std::vector<float> *precip, std::vector<float> *pet, std::vector<float> *fastFlow, std::vector<float> *slowFlow, std::vector<float> *soilMoisture) {
    ...
    soilMoisture->at(i) = 0;
		//discharge->at(i) = (cNode->dischargeQF + cNode->dischargeSF) * node->area * 0.277777777777778f / stepHours; // Convert from mm/time to cms
	}

	return true;
}

void HyMOD::WaterBalanceInt(GridNode *node, HyMODGridNode *cNode, float stepHours, float precipIn, float petIn) {

	float precip = precipIn * stepHours * cNode->params[PARAM_HYMOD_PRECIP]; // precipIn is mm/hr, precip is mm
	...
	float OV2 = std::max(0.0f, precip + cNode->XHuz - cNode->params[PARAM_HYMOD_HUZ]); // Compute OV2 if enough PP
	float PPinf = precip - OV2; // PP that does not go to OV2
	float Hint = std::min(cNode->params[PARAM_HYMOD_HUZ], PPinf + cNode->XHuz); // Intermediate height
	float Cint = cNode->CPar * (1 - pow(1 - Hint / cNode->params[PARAM_HYMOD_HUZ], 1 + cNode->b)); // Intermediate contents
	...
	float Hend = cNode->params[PARAM_HYMOD_HUZ] * ( 1 - pow(1 - Cend / cNode->CPar, 1 / (1 + cNode->b))); // Final height corresponding to SMA contents
...
void HyMOD::LocalRouteQF(GridNode *node, HyMODGridNode *cNode, float stepHours) {
...
void HyMOD::LocalRouteSF(GridNode *node, HyMODGridNode *cNode, float stepHours) {
...
void HyMOD::InitializeParameters(std::map<GaugeConfigSection *, float *> *paramSettings) {
	...
	for (std::vector<GridNode>::iterator itr = nodes->begin(); itr != nodes->end(); itr++) {
		...
		memcpy(cNode->params, (*paramSettings)[node->gauge], sizeof(float)*PARAM_HYMOD_QTY);
		...
		cNode->XHuz = cNode->params[PARAM_HYMOD_HUZ] * (1 - pow((1 - cNode->XCuz / cNode->CPar), 1 / (1 + cNode->b)));
