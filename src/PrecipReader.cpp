...
bool PrecipReader::Read(char *file, SUPPORTED_PRECIP_TYPES type, std::vector<GridNode> *nodes, std::vector<float> *currentPrecip, float precipConvert, std::vector<float> *prevPrecip, bool hasQPF) {
	if (!strcmp(lastPrecipFile, file)) {
		...
		return true; // This is the same precip file that we read last time, we assume currentPrecip is still valid!
	}

	if (!hasQPF) {
		// Update this here so we recheck for missing files & don't recheck for forecast precip
	...
	// We have two options now... Either the precip grid & the basic grids are the same
	...
	if (g_DEM->IsSpatialMatch(precipGrid)) {
		...
		for (size_t i = 0; i < nodes->size(); i++) {
			...
			if (precipGrid->data[node->y][node->x] != precipGrid->noData && precipGrid->data[node->y][node->x] > 0.0) {
		...
		for (size_t i = 0; i < nodes->size(); i++) {
			GridLoc pt;
                        GridNode *node = &(nodes->at(i));
			if (precipGrid->GetGridLoc(node->refLoc.x, node->refLoc.y, &pt) && precipGrid->data[pt.y][pt.x] != precipGrid->noData && precipGrid->data[pt.y][pt.x] > 0.0) {
