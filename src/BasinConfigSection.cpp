...
BasinConfigSection::BasinConfigSection(char *newName) {
	strcpy(name, newName);
}
...
CONFIG_SEC_RET BasinConfigSection::ProcessKeyValue(char *name, char *value) {
	if (strcasecmp(name, "gauge") == 0) {
		...
		ERROR_LOGF("Unknown key value \"%s=%s\" in basin %s!", name, value, this->name);
      return INVALID_RESULT;
	}
...
bool BasinConfigSection::IsDuplicate(char *name) {
        std::map<std::string, BasinConfigSection *>::iterator itr = g_basinConfigs.find(std::string(name));
