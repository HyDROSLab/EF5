...
CONFIG_SEC_RET TaskConfigSection::ProcessKeyValue(char *name, char *value) {
    ...
    INFO_LOGF("Valid run style options are \"%s\"",  "SIMU, SIMU_RP, CALI_ARS, CALI_DREAM, CLIP_BASIN, CLIP_GAUGE, BASIN_AVG");
    ...
    tempFSet = true;
		...
		if (!modelSet) {
			ERROR_LOG("The MODEL setting must be specified before the PARAM_SET setting in the task!");
			return INVALID_RESULT;
		}
    TOLOWER(value);
    std::map<std::string, ParamSetConfigSection *>::iterator itr = g_paramSetConfigs[model].find(value);
    ...
    paramsSet = true;
		...
		if (!routeSet) {
			ERROR_LOG("The ROUTING setting must be specified before the ROUTING_PARAM_SET setting in the task!");
			return INVALID_RESULT;
		}
    TOLOWER(value);
    std::map<std::string, RoutingParamSetConfigSection *>::iterator itr = g_routingParamSetConfigs[routing].find(value);
    ...
    if (!snowSet) {
      ERROR_LOG("The SNOW setting must be specified before the SNOW_PARAM_SET setting in the task!");
      return INVALID_RESULT;
    }
    TOLOWER(value);
    std::map<std::string, SnowParamSetConfigSection *>::iterator itr = g_snowParamSetConfigs[snow].find(value);
    ...
    if (!inundationSet) {
      ERROR_LOG("The INUNDATION setting must be specified before the INUNDATION_PARAM_SET setting in the task!");
      return INVALID_RESULT;
    }
    TOLOWER(value);
    std::map<std::string, InundationParamSetConfigSection *>::iterator itr = g_inundationParamSetConfigs[inundation].find(value);
    ...
    timeEndSet = true;
		...
		if (!modelSet) {
      ERROR_LOG("The MODEL setting must be specified before the CALI_PARAM setting in the task!");
      return INVALID_RESULT;
    }
		TOLOWER(value);
    std::map<std::string, CaliParamConfigSection *>::iterator itr = g_caliParamConfigs[model].find(value);
    ...
    if (!routeSet) {
      ERROR_LOG("The ROUTING setting must be specified before the ROUTING_CALI_PARAM setting in the task!");
      return INVALID_RESULT;
    }
    TOLOWER(value);
    std::map<std::string, RoutingCaliParamConfigSection *>::iterator itr = g_routingCaliParamConfigs[routing].find(value);
    ...
    if (!snowSet) {
      ERROR_LOG("The SNOW setting must be specified before the SNOW_CALI_PARAM setting in the task!");
      return INVALID_RESULT;
    }
    TOLOWER(value);
    std::map<std::string, SnowCaliParamConfigSection *>::iterator itr = g_snowCaliParamConfigs[snow].find(value);
    ...
    if (!inundationSet) {
      ERROR_LOG("The INUNDATION setting must be specified before the INUNDATION_CALI_PARAM setting in the task!");
      return INVALID_RESULT;
    }
    TOLOWER(value);
    std::map<std::string, InundationCaliParamConfigSection *>::iterator itr = g_inundationCaliParamConfigs[inundation].find(value);
...
CONFIG_SEC_RET TaskConfigSection::ValidateSection() {
  ...
  if (IsCalibrationRunStyle(style)) {
  	...
  	if (snowSet && !snowCaliParamSet) {
		...
		if (!foundWB) {
			ERROR_LOG("The calibration gauge (water balance cali param) was not found in the basin!");
			return INVALID_RESULT;
		}
		if (!foundR) {
			ERROR_LOG("The calibration gauge (routing cali param) was not found in the basin!");
    	return INVALID_RESULT;
  	}
		if (snowSet && !foundS) {
			ERROR_LOG("The calibration gauge (snow cali param) was not found in the basin!");
    	return INVALID_RESULT;
  	}
	}
