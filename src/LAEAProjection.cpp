...
float LAEAProjection::GetLen(float x, float y, FLOW_DIR dir) {
	...
	switch (dir) {
		...
		default:
			return 0; // We would do __builtin_unreachable(); but Apple's GCC lags behind
	}
...
float LAEAProjection::GetArea(float x, float y) {
...
void LAEAProjection::ReprojectPoint(float lon, float lat, float *x, float *y) {
	...
	float kPrime =  sqrt(2.0 / (1.0 + sin(stan_par) * sin(lat) + cos(stan_par) * cos(lat) * cos(lon - cent_lon)));
	float projX = kPrime * cos(lat) * sin(lon - cent_lon);
	float projY = kPrime * (cos(stan_par) * sin(lat) - sin(stan_par) * cos(lat) * cos(lon - cent_lon));
...
void LAEAProjection::UnprojectPoint(float x, float y, float *lon, float *lat) {
	...
	float geoLon = cent_lon +  atan( (projX * sin(c)) / ( rho * cos(stan_par) * cos(c) - projY * sin(stan_par) * sin(c)) );
