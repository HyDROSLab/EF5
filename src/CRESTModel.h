...
class CRESTModel : public WaterBalanceModel {
	...
	public:
		...
		bool WaterBalance(float stepHours, std::vector<float> *precip, std::vector<float> *pet, std::vector<float> *fastFlow, std::vector<float> *slowFlow, std::vector<float> *soilMoisture);
		bool IsLumped() { return false; }
		const char *GetName() { return "crest"; }

	private:
		void WaterBalanceInt(GridNode *node, CRESTGridNode *cNode, float stepHours, float precipIn, float petIn, float *fastFlow, float *slowFlow);
