...
InundationParamSetConfigSection::InundationParamSetConfigSection(char *nameVal, INUNDATIONS inundationVal) {
	strcpy(name, nameVal);
	currentGauge = NULL;
	currentParams = NULL;
		...
		if (currentGauge != NULL) {
			...
			for (int i = 0; i < numParams; i++) {
				if (!currentParamsSet[i]) {
					ERROR_LOGF("Incomplete parameter set! Parameter \"%s\" was not given a value.", inundationParamStrings[inundation][i]);
					return INVALID_RESULT;
				}
			}	
...
bool InundationParamSetConfigSection::IsDuplicate(char *name, INUNDATIONS inundationVal) {
        std::map<std::string, InundationParamSetConfigSection *>::iterator itr = g_inundationParamSetConfigs[inundationVal].find(name);
