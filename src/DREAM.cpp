...
void DREAM::Initialize(CaliParamConfigSection *caliParamConfigNew, RoutingCaliParamConfigSection *routingCaliParamConfigNew, SnowCaliParamConfigSection *snowCaliParamConfigNew, int numParamsWBNew, int numParamsRNew, int numParamsSNew, Simulator *simNew) {
  ...
  INFO_LOGF("num params is %i (water balance :%i, routing: %i, snow: %i)", numParams, numParamsWB, numParamsR, numParamsS);
  ...
  strcpy(pointerMCMC->outlierTest, "IQR_Test");       // What kind of test to detect outlier chains?
  
  float newdraw = floorf((float)pointerMCMC->ndraw / (float)(pointerMCMC->seq*pointerMCMC->steps) + 0.5) * (pointerMCMC->seq*pointerMCMC->steps);
...
void DREAM::Initialize(CaliParamConfigSection *caliParamConfigNew, int numParamsNew, float *paramMins, float *paramMaxs, std::vector<Simulator> *ensSimsNew, std::vector<int> *paramsPerSimNew) {
  caliParamConfig = caliParamConfigNew;
...
void DREAM::CalibrateParams() {
  ...
  //steps = pointerMCMC->steps; //Steps will change, need to kepp original value for memory deallocation
  //------Start Messages-----------------------------------------------------------//
  INFO_LOGF("%s", "DiffeRential Evolution Adaptive Metropolis (DREAM) Algorithm");
  
#if _OPENMP
  if (omp_get_max_threads() > 1) {
    INFO_LOGF("Running in parallel mode with %i threads", omp_get_max_threads());
  } else {
  ...
  //------Initialize Variables: Call InitVar routine-------------------------------//
  InitVar(pointerMCMC, &pointerRUNvar, &pointerOutput);
  //------Check for Successful Memory Allocation-----------------------------------//
  MEMORYCHECK(pointerRUNvar, "at dream.c: Memory Allocation for DREAM struct run variable not successfull\n");
  ...
  //if Extra.InitPopulation = 'LHS_BASED'
  //Latin hypercube sampling when indicated
  allocate2D(&x, pointerMCMC->seq, pointerInput->nPar);
  LHSU(&x, pointerInput->nPar, pointerInput->ParRangeMax, pointerInput->ParRangeMin, pointerMCMC->seq);
  ...
  MEMORYCHECK(delta_tot, "at dream.c: Memory Allocation for DREAM variable delta_tot not successfull\n");
  ...
  Gelman(pointerOutput->R_stat, 0, pointerRUNvar->Sequences, pointerRUNvar->iloc, pointerMCMC->n, pointerMCMC->seq, 0);
  
  //Allocate Variables
  allocate2D(&x_old, pointerMCMC->seq, pointerMCMC->n);
  MEMORYCHECK(x_old, "at dream.c: Memory Allocation for DREAM variable x_old not successfull\n");
  p_old = (float*)malloc(pointerMCMC->seq*sizeof(float*));
  MEMORYCHECK(p_old, "at dream.c: Memory Allocation for DREAM variable p_old not successfull\n");
  log_p_old = (float*)malloc(pointerMCMC->seq*sizeof(float*));
  MEMORYCHECK(log_p_old, "at dream.c: Memory Allocation for DREAM variable log_p_old not successfull\n");
  allocate2D(&x_new, pointerMCMC->seq, pointerMCMC->n);
  MEMORYCHECK(x_new, "at dream.c: Memory Allocation for DREAM variable x_new not successfull\n");
  allocate2D(&p_xnew, pointerMCMC->seq, 2);
  MEMORYCHECK(p_xnew, "at dream.c: Memory Allocation for DREAM variable p_xnew not successfull\n");
  log_p_xnew = (float*)malloc(pointerMCMC->seq*sizeof(float));
  MEMORYCHECK(log_p_xnew, "at dream.c: Memory Allocation for DREAM variable log_p_xnew not successfull\n");
  alpha12 = (float*)malloc(pointerMCMC->seq*sizeof(float*));
  MEMORYCHECK(alpha12, "at dream.c: Memory Allocation for DREAM variable alpha12 not successfull\n");
  accept = (float*)malloc(pointerMCMC->seq*sizeof(float*));
  MEMORYCHECK(accept, "at dream.c: Memory Allocation for DREAM variable accept not successfull\n");
  
  // Preallocate memory needed here
  allocate2D(&newgen, pointerMCMC->seq, pointerMCMC->n + 2);
  MEMORYCHECK(newgen, "at dream.c: Memory Allocation for DREAM variable newgen not successfull\n");
  allocate2D(&r, pointerMCMC->seq, pointerMCMC->n + 2);
  MEMORYCHECK(r, "at dream.c: Memory Allocation for DREAM variable r not successfull\n");
  std_array = (float*)malloc(pointerMCMC->seq*sizeof(float));
  MEMORYCHECK(std_array, "at dream.c: Memory Allocation for DREAM variable std_array not successfull\n");
  delta_normX = (float*)malloc(pointerMCMC->seq*sizeof(float));
  MEMORYCHECK(delta_normX, "at dream.c: Memory Allocation for DREAM variable delta_normX not successfull\n");
  dnX_array = (float*)malloc((pointerMCMC->n+2)*sizeof(float));
  MEMORYCHECK(dnX_array, "at dream.c: Memory Allocation for DREAM variable dnX_array not successfull\n");
  ...
  while ((pointerRUNvar->Iter < pointerMCMC->ndraw) && !converged) {
    ...
    for (gen_number = 0; gen_number < pointerMCMC->steps; gen_number++) {
      ...
      offde(x_new, x_old, X, pointerRUNvar->CR, pointerMCMC, pointerRUNvar->Table_JumpRate, pointerInput, "Reflect", R2, "No");
      ...
      metrop(newgen, alpha12, accept, x_new, p_xnew, log_p_xnew, x_old, p_old, log_p_old, pointerInput, pointerMCMC, 3);
      
      //Check whether we do delayed rejection or not
      //If DR = "Yes", then do compute several things. For this implementation of DREAM,
      //We skipped said computations (i.e. DR = "No")
      ...
      for (i=0;i <  pointerMCMC->n + 2;i++) {
        for (j=0;j <  pointerMCMC->seq;j++) {
          pointerRUNvar->Sequences[pointerRUNvar->iloc-1][i][j] = t_newgen[i][j];
        }
      }
      ...
      //Update hist_logp
      pointerRUNvar->hist_logp[pointerRUNvar->counter-1][0] = pointerRUNvar->Iter;
      
      for (j = 1; j < pointerMCMC->seq+1; j++) {
        pointerRUNvar->hist_logp[pointerRUNvar->counter-1][j] = X[j-1][pointerMCMC->n+1];
      }
      
      //Save some important output -- Acceptance Rate
      pointerOutput->AR[pointerRUNvar->counter-1][0] = pointerRUNvar->Iter;
      //Next Line, variable accept2 not included in this implementation of DREAM since Extra.DR = 'No'
      pointerOutput->AR[pointerRUNvar->counter-1][1] = 100.0 * (sumarray(accept,pointerMCMC->seq,1) / (float)((pointerMCMC->seq + ItExtra)));
    ...
    //-----------End of: for (gen_number=0;gen_number < 1;gen_number++)----------------------- //
    ...
    for (i = 1; i < pointerMCMC->nCR + 1; i++) {
      pointerOutput->CR[pointerRUNvar->teller-1][i] = pointerRUNvar->pCR[0][i-1];
    }
    ...
    Gelman(pointerOutput->R_stat, pointerRUNvar->teller-1, pointerRUNvar->Sequences, (end_loc-start_loc+1), pointerMCMC->n, pointerMCMC->seq, start_loc);
    pointerOutput->R_stat[pointerRUNvar->teller-1][0] = pointerRUNvar->Iter;
    
    // Lets see if we converged or not
    converged = true;
    NORMAL_LOGF("R_stat: %f ", pointerOutput->R_stat[pointerRUNvar->teller-1][0]);
    for (j = 0; j < pointerMCMC->n; j++) {
      NORMAL_LOGF("%f ", pointerOutput->R_stat[pointerRUNvar->teller-1][j + 1]);
      if (pointerOutput->R_stat[pointerRUNvar->teller-1][j + 1] > 1.2 || pointerOutput->R_stat[pointerRUNvar->teller-1][j + 1] == -2.0) {
        converged = false;
      }
    }
    NORMAL_LOGF("%s", "\n");
  ...
  MEMORYCHECK(post_array, "at dream.c: Memory Allocation for DREAM variable post_array not successfull\n");
  ...
  /*deallocate2D(&pointerRUNvar->hist_logp, pointerRUNvar->Nelem - 1 + 10);
   ...
   deallocate2D(&pointerOutput->R_stat, floorf((pointerRUNvar->Nelem+10) / steps));
   free(pointerRUNvar->lCR);
   free(pointerOutput);
   //free(pointerRUNvar);
...
void DREAM::CompDensity(float **p, float *log_p, float **x, struct DREAM_Parameters *MCMC, struct Model_Input *Input, int option) {
...
#pragma omp parallel for
    ...
    for (i = 0; i < count; i++) {
      ...
      for (int j = 0; j < numEns; j++) {
        dischargeVals[j] = ensSims->at(j).SimulateForCaliTS(&(x[i][paramIndex]));
      ...
      for (int j = 0; j < numObs; j++) {
        ...
        //printf("d: %f %f %f\n", currentDischargeSet[0], currentDischargeSet[1], currentDischargeSet[2]);
      ...
      float distMax = bin_tally[0], distExpected = 1.0 / ((float)(numEns) + 1.0);
      for (int z = 0; z < numEns + 1; z++) {
        if (distMax < bin_tally[z]) {
          distMax = bin_tally[z];
        }
      }
      //printf("t: %i %i %i %i\n", bin_tally[0], bin_tally[1], bin_tally[2], bin_tally[3]);
      distMax /= ((float)(numObs));
      objScore = fabs((distMax - distExpected)/distExpected) * 100.0;
      //printf("%f %f %f %f\n", distExpected, distMax, objScore, (float)(numObs));
      p[i][0] = -1 * objScore;
      p[i][1] = i;
      log_p[i] = -0.5 * objScore;
