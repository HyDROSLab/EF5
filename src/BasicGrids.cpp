...
static int FlowsIn(long nextX, long nextY, FLOW_DIR dir, long currentHeight, GridLoc *locIn, long maxSearch, GridLoc *locOut);
...
bool LoadBasicGrids() {
	...
	if (g_basicConfig->IsESRIDDM()) {
		if (CheckESRIDDM()) {
			...
			ERROR_LOG("Was expecting an ESRI Drainage Direction Map and got invalid values!");
			return false;
		}
	} else {
		if (!CheckSimpleDDM()) {
			ERROR_LOG("Was expecting a simple Drainage Direction Map and got invalid values!");
			return false;
		}
	}
...
void FindIndBasins(float left, float right, float top, float bottom) {
  ...
  long minY = LONG_MAX;
    ...
    for (long col = minX; col < maxX; col++) {
			...
			if (g_DEM->data[row][col] == g_DEM->noData || g_FAM->data[row][col] == g_FAM->noData) {
	...
	for (FAMSearch *currentFS = gridCells.front(); currentFS != NULL; currentFS = gridCells.front()) {
      ...
      fprintf(test, "[Gauge %i] cellx=%li celly=%li outputts=false #Num Cells = %f\n", totalGauges, cCell->x, cCell->y, g_FAM->data[cCell->y][cCell->x]);
        ...
        if (TestUpstream(cCell->x, cCell->y, (FLOW_DIR)i, &locN)) {
          ...
          nextN->y = locN.y;
					...
					if (index < gcItrs.size() && g_DEM->data[nextN->y][nextN->x] != g_DEM->noData && g_FAM->data[nextN->y][nextN->x] != g_FAM->noData) {
						...
						//printf("Hmm %i (%f, %f, %f)\n", index, g_DEM->data[nextN->y][nextN->x], g_FAM->data[nextN->y][nextN->x], g_DDM->data[nextN->y][nextN->x]);
						delete nextN;
					}
        }
      }
...
void ClipBasicGrids(BasinConfigSection *basin, std::vector<GridNode> *nodes, const char *name, const char *output) {
  ...
  for (long i = 0; i < grid.numRows; i++) {
    ...
    GridNode *node = &(*itr);
    	...
    	if (gauge->GetGridNodeIndex() == (long)node->index) {
				printf("[gauge %s] cellx=%li celly=%li\n", gauge->GetName(), node->x - minX, node->y - minY);
				break;
			}
		}
	...
	char buffer[255];
	sprintf(buffer, "%s/%s.%s", output, strrchr(g_basicConfig->GetDEM(), '/'), name);
	...
	sprintf(buffer, "%s/%s.%s", output, strrchr(g_basicConfig->GetDDM(), '/'), name);
  WriteFloatTifGrid(buffer, &grid);
  ...
  sprintf(buffer, "%s/%s.%s", output, strrchr(g_basicConfig->GetFAM(), '/'), name);
  WriteFloatTifGrid(buffer, &grid);
...
void FixFlowDir(BasinConfigSection *basin, std::vector<GridNode> *nodes) {
	...
	int maxDist = 20000.0 / g_Projection->GetLen(gauge->GetLon(), gauge->GetLat(), FLOW_NORTH); //3000;
	if (maxDist < 2) {
		maxDist = 2;
	}
    ...
    GridNode *node = &(*itr);
		...
		for (int dist = 1; dist < maxDist; dist++) {
			testX = node->x + 1 * dist;
			testY = node->y;
			if (testX < g_FAM->numCols && g_FAM->data[testY][testX] != g_FAM->noData && g_FAM->data[testY][testX] > maxFlow && g_DEM->data[node->y][node->x] == g_DEM->data[testY][testX]) {
				float thisDist = powf(testY - node->y, 2.0) + powf(testX - node->x, 2.0);
				if (thisDist < minDist) {
					minDist = thisDist;
			...
			testX = node->x + 1 * dist;
      testY = node->y - 1 * dist;
      if (testX < g_FAM->numCols && testY >= 0 && g_FAM->data[testY][testX] != g_FAM->noData && g_FAM->data[testY][testX] > maxFlow && g_DEM->data[node->y][node->x] == g_DEM->data[testY][testX]) {
        float thisDist = powf(testY - node->y, 2.0) + powf(testX - node->x, 2.0);
        if (thisDist < minDist) {
					minDist = thisDist;
					maxFlow = g_FAM->data[testY][testX];
			...
			testX = node->x;
      testY = node->y - 1 * dist;
      if (testY >= 0 && g_FAM->data[testY][testX] != g_FAM->noData && g_FAM->data[testY][testX] > maxFlow && g_DEM->data[node->y][node->x] == g_DEM->data[testY][testX]) {
        float thisDist = powf(testY - node->y, 2.0) + powf(testX - node->x, 2.0);
        if (thisDist < minDist) {
					minDist = thisDist;
					maxFlow = g_FAM->data[testY][testX];
			...
			testX = node->x - 1 * dist;
      testY = node->y - 1 * dist;
      if (testX >= 0 && testY >= 0 && g_FAM->data[testY][testX] != g_FAM->noData && g_FAM->data[testY][testX] > maxFlow && g_DEM->data[node->y][node->x] == g_DEM->data[testY][testX]) {
        float thisDist = powf(testY - node->y, 2.0) + powf(testX - node->x, 2.0);
        if (thisDist < minDist) {
					minDist = thisDist;
					maxFlow = g_FAM->data[testY][testX];
			...
			testX = node->x - 1 * dist;
      testY = node->y;
      if (testX >= 0 && g_FAM->data[testY][testX] != g_FAM->noData && g_FAM->data[testY][testX] > maxFlow && g_DEM->data[node->y][node->x] == g_DEM->data[testY][testX]) {
        float thisDist = powf(testY - node->y, 2.0) + powf(testX - node->x, 2.0);
        if (thisDist < minDist) {
					minDist = thisDist;
					maxFlow = g_FAM->data[testY][testX];
			...
			testX = node->x - 1 * dist;
      testY = node->y + 1 * dist;
      if (testX >= 0 && testY < g_FAM->numRows && g_FAM->data[testY][testX] != g_FAM->noData && g_FAM->data[testY][testX] > maxFlow && g_DEM->data[node->y][node->x] == g_DEM->data[testY][testX]) {
        float thisDist = powf(testY - node->y, 2.0) + powf(testX - node->x, 2.0);
        if (thisDist < minDist) {
					minDist = thisDist;
					maxFlow = g_FAM->data[testY][testX];
			...
			testX = node->x;
      testY = node->y + 1 * dist;
      if (testY < g_FAM->numRows && g_FAM->data[testY][testX] != g_FAM->noData && g_FAM->data[testY][testX] > maxFlow && g_DEM->data[node->y][node->x] == g_DEM->data[testY][testX]) {
        float thisDist = powf(testY - node->y, 2.0) + powf(testX - node->x, 2.0);
        if (thisDist < minDist) {
					minDist = thisDist;
					maxFlow = g_FAM->data[testY][testX];
			...
			testX = node->x + 1 * dist;
      testY = node->y + 1 * dist;
      if (testX < g_FAM->numCols && testY < g_FAM->numRows && g_FAM->data[testY][testX] != g_FAM->noData && g_FAM->data[testY][testX] > maxFlow && g_DEM->data[node->y][node->x] == g_DEM->data[testY][testX]) {
        float thisDist = powf(testY - node->y, 2.0) + powf(testX - node->x, 2.0);
        if (thisDist < minDist) {
					minDist = thisDist;	
					maxFlow = g_FAM->data[testY][testX];
		...
		printf("Old dir %f, new dir %i\n", g_DDM->data[node->y][node->x], flowDir);
...
void CarveBasin(BasinConfigSection *basin, std::vector<GridNode> *nodes, std::map<GaugeConfigSection *, float *> *inParamSettings, std::map<GaugeConfigSection *, float *> *outParamSettings, GaugeMap *gaugeMap, float *defaultParams, std::map<GaugeConfigSection *, float *> *inRouteParamSettings, std::map<GaugeConfigSection *, float *> *outRouteParamSettings, float *defaultRouteParams, std::map<GaugeConfigSection *, float *> *inSnowParamSettings, std::map<GaugeConfigSection *, float *> *outSnowParamSettings, float *defaultSnowParams, std::map<GaugeConfigSection *, float *> *inInundationParamSettings, std::map<GaugeConfigSection *, float *> *outInundationParamSettings, float *defaultInundationParams) {
	...
	// Figure out where each gauge is at and get the flow accumulation from the grid for it.
	// Also resets the used flag to false
	for (std::vector<GaugeConfigSection *>::iterator itr = gauges->begin(); itr != gauges->end(); ) {
		...
		float areaFactor = 1.0 / g_Projection->GetArea(gauge->GetLon(), gauge->GetLat());
		int maxDist = 20000.0 / g_Projection->GetLen(gauge->GetLon(), gauge->GetLat(), FLOW_NORTH); //3000;
		if (maxDist < 2) {
			maxDist = 2;
		}
		...
		if (gauge->HasObsFlowAccum()) {
			GridLoc minLoc;
      float testError, minError = pow((float)(g_FAM->data[loc->y][loc->x]) - gauge->GetObsFlowAccum()*areaFactor, 2.0);
			minLoc.x = loc->x;
			minLoc.y = loc->y;
			long testX, testY;
			for (int dist = 1; dist < maxDist; dist++) {
        ...
        if (testX < g_FAM->numCols && g_FAM->data[testY][testX] != g_FAM->noData) {
          testError = pow((float)(g_FAM->data[testY][testX]) - gauge->GetObsFlowAccum()*areaFactor, 2.0);
    ...
    unsigned long index = loc->y * g_DEM->numCols + loc->x;
		std::map<unsigned long, GaugeConfigSection *>::iterator itrM = gaugeCMap.find(index);
    if (itrM == gaugeCMap.end()) {
      gaugeCMap.insert(std::pair<unsigned long, GaugeConfigSection*>(index, gauge));
			itr++;
		} else {
	...
	// Here we compute which grid cells & gauges are upstream of our independent gauges
	for (GaugeConfigSection *currentGauge = (*gauges)[0]; currentGauge != NULL; currentGauge = NextUnusedGauge(gauges)) {
    
		std::map<GaugeConfigSection *, float *>::iterator pitr = inParamSettings->find(currentGauge);
		if (pitr == inParamSettings->end() && !defaultParams) {
			ERROR_LOGF("Independent basin \"%s\" lacks a water balance parameter set!", currentGauge->GetName());
			return;
		}
    
		if (pitr == inParamSettings->end()) {
			outParamSettings->insert(std::pair<GaugeConfigSection *, float *>(currentGauge, defaultParams));
		} else {
			outParamSettings->insert(std::pair<GaugeConfigSection *, float *>(pitr->first, pitr->second));
		}
   
		if (inRouteParamSettings) { 
    	pitr = inRouteParamSettings->find(currentGauge);
			if (pitr == inRouteParamSettings->end() && !defaultRouteParams) {
				ERROR_LOGF("Independent basin \"%s\" lacks a routing parameter set!", currentGauge->GetName());
				return;
			}
    
			if (pitr == inRouteParamSettings->end()) {
				outRouteParamSettings->insert(std::pair<GaugeConfigSection *, float *>(currentGauge, defaultRouteParams));
			} else {
				outRouteParamSettings->insert(std::pair<GaugeConfigSection *, float *>(pitr->first, pitr->second));
			}
    ...
    if (pitr == inSnowParamSettings->end() && !defaultSnowParams) {
      ERROR_LOGF("Independent basin \"%s\" lacks a snow parameter set!", currentGauge->GetName());
      return;
    }

    if (pitr == inSnowParamSettings->end()) {
      outSnowParamSettings->insert(std::pair<GaugeConfigSection *, float *>(currentGauge, defaultSnowParams));
    } else {
      outSnowParamSettings->insert(std::pair<GaugeConfigSection *, float *>(pitr->first, pitr->second));
    }
    ...
    if (pitr == inInundationParamSettings->end() && !defaultInundationParams) {
      ERROR_LOGF("Independent basin \"%s\" lacks a inundation parameter set!", currentGauge->GetName());
      return;
    }

    if (pitr == inInundationParamSettings->end()) {
      outInundationParamSettings->insert(std::pair<GaugeConfigSection *, float *>(currentGauge, defaultInundationParams));
    } else {
      outInundationParamSettings->insert(std::pair<GaugeConfigSection *, float *>(pitr->first, pitr->second));
    }
      ...
      GaugeConfigSection *prevGauge = currentN->gauge;
			...
			if (nodeGauge) {
				...
				// Add this to the gauge map which allows us to figure out upstream gauges & contributions
				gaugeMap->AddUpstreamGauge(prevGauge, nodeGauge);
        
				//We fill in outParamSettings for the new gauge here
				//Since it is not required that inParamSettings contain parameters for every gauge in the basin we will in parameters from downstream gauges upstream
				pitr = inParamSettings->find(nodeGauge);
        if (pitr == inParamSettings->end()) {
          pitr = outParamSettings->find(prevGauge);
					outParamSettings->insert(std::pair<GaugeConfigSection *, float *>(nodeGauge, pitr->second));
				} else {
          outParamSettings->insert(std::pair<GaugeConfigSection *, float *>(pitr->first, pitr->second));
				}
				if (inRouteParamSettings) {
        	...
        	if (pitr == inRouteParamSettings->end()) {
          	pitr = outRouteParamSettings->find(prevGauge);
						outRouteParamSettings->insert(std::pair<GaugeConfigSection *, float *>(nodeGauge, pitr->second));
					} else {
         	 outRouteParamSettings->insert(std::pair<GaugeConfigSection *, float *>(pitr->first, pitr->second));
					}
				}
        ...
        if (inSnowParamSettings) {
          ...
          if (pitr == inSnowParamSettings->end()) {
            ...
            outSnowParamSettings->insert(std::pair<GaugeConfigSection *, float *>(nodeGauge, pitr->second));
          } else {
            outSnowParamSettings->insert(std::pair<GaugeConfigSection *, float *>(pitr->first, pitr->second));
          }
        }
          ...
          if (pitr == inInundationParamSettings->end()) {
            ...
            outInundationParamSettings->insert(std::pair<GaugeConfigSection *, float *>(nodeGauge, pitr->second));
          } else {
            outInundationParamSettings->insert(std::pair<GaugeConfigSection *, float *>(pitr->first, pitr->second));
          }
        }
	...
	for (std::vector<GaugeConfigSection *>::iterator itr = gauges->begin(); itr != gauges->end(); itr++ ) {
	GaugeConfigSection *gauge = *(itr);
	GridLoc *loc = gauge->GetGridLoc();
	fprintf(fp, "[Gauge %s] cellx=%li celly=%li basinarea=%f %ld %li wantco=true outputts=false\n", gauge->GetName(), loc->x, loc->y, nodes->at(gauge->GetGridNodeIndex()).contribArea, gauge->GetFlowAccum(), gauge->GetGridNodeIndex());
	fprintf(fp2, "gauge=%s\n", gauge->GetName());
	}
	fclose(fp);
	fclose(fp2);
...
bool GetDownstreamHeight(long x, long y, long *outsideHeight) {
  ...
  if (nextX >= 0 && nextY >= 0 && nextX < g_DEM->numCols && nextY < g_DEM->numRows && g_DEM->data[nextY][nextX] != g_DEM->noData) {
...
GaugeConfigSection *FindGauge(std::map<unsigned long, GaugeConfigSection *> *gauges, GridNode *node) {
	...
	std::map<unsigned long, GaugeConfigSection *>::iterator itr = gauges->find(index);
		...
		for (long col = 0; col < g_DDM->numCols; col++) {
			...
			switch ((int)(g_DDM->data[row][col])) {
				...
				case 128:
					continue;
				default:
					ERROR_LOGF("Bad DDM value %i at (%li, %li) %f", (int)(g_DDM->data[row][col]), col, row, g_DDM->noData);
					return false;
			}
		}
	}
...
void FixFAM() {
    ...
    for (long col = 0; col < g_DEM->numCols; col++) {
			if (g_DEM->data[row][col] == g_DEM->noData || g_FAM->data[row][col] == g_FAM->noData || g_DDM->data[row][col] == g_DDM->noData) {
    ...
    for (long col = 0; col < g_FAM->numCols; col++) {
          ...
          if (TestUpstreamBroken(col, row, (FLOW_DIR)i, &locN)) {
						INFO_LOGF("Adding %f cells to accumulation for cell (%li, %li; %f)\n", g_FAM->data[row][col], col, row, g_FAM->data[locN.y][locN.x]);
						g_FAM->data[locN.y][locN.x] += g_FAM->data[row][col];
						if (!g_basicConfig->IsSelfFAM()) {
							g_FAM->data[locN.y][locN.x] += 1;
						}
					}
				}
			}
...
int FlowsIn(long nextX, long nextY, FLOW_DIR dir, long currentHeight, GridLoc *locIn, long maxSearch, GridLoc *locOut) {
  ...
  if (nextX >= 0 && nextY >= 0 && nextX < g_DEM->numCols && nextY < g_DEM->numRows) {
		if (labs(nextX - locIn->x) > maxSearch || labs(nextY - locIn->y) > maxSearch) {
    ...
    for (long col = 0; col < g_DEM->numCols; col++) {
          ...
          walkNodes.pop();
					...
					for (int i = 1; i < FLOW_QTY; i++) {
						...
						if (FlowsIn(currentN->x, currentN->y, (FLOW_DIR)i, g_DEM->data[currentN->y][currentN->x], &locIn, 5, &locOut)) {
				...
				//if (col >= 1 && row >= 1)
				//printf("Found inflow sink %i %i %i (%i %i %i %i %i %i %i %i)!\n", col, row, g_DEM->data[row][col], g_DEM->data[row-1][col-1], g_DEM->data[row-1][col], g_DEM->data[row-1][col+1], g_DEM->data[row][col-1], g_DEM->data[row][col+1], g_DEM->data[row+1][col-1], g_DEM->data[row+1][col], g_DEM->data[row+1][col+1]);
			}
    }
  }
