...
void InitVar(struct DREAM_Parameters *pstPar, struct DREAM_Variables **pstRUN, struct DREAM_Output **pstOutput)
{
//Initializes important variables for use in the algorithm
 ...
 buffer = 10; //Matlab version allows arrays to increase size after pre-allocation
 ...
 *pstRUN = (struct DREAM_Variables *)malloc(sizeof(struct DREAM_Variables));
 ...
 (*pstRUN)->Nelem = floorf(pstPar->ndraw/pstPar->seq) + 1;
  INFO_LOGF("Nelem is %i (%f), ndraw is %li", (*pstRUN)->Nelem, floorf(1.25 * (float)((*pstRUN)->Nelem)), pstPar->ndraw); 
 //Allocate the array that contains the history of the log_density of each chain
 allocate2D(&(*pstRUN)->hist_logp, (*pstRUN)->Nelem - 1 + buffer * 2, pstPar->seq+1);
 ...
 *pstOutput = (struct DREAM_Output *)malloc(sizeof(struct DREAM_Output));
 ...
 (*pstOutput)->AR[1][1] = pstPar->seq - 1; (*pstOutput)->AR[1][2] = pstPar->seq - 1;
 ...
 (*pstRUN)->Sequences = (float ***)malloc(floorf(1.25 * (float)((*pstRUN)->Nelem))*sizeof(float **));
 for (i=0;i < (floorf(1.25 * (float)((*pstRUN)->Nelem)));i++) 
 {
  (*pstRUN)->Sequences[i] = (float **)malloc((pstPar->n + 2)*sizeof(float *));
  ...
  MEMORYCHECK((*pstRUN)->Sequences,"ERROR at InitVar: Out of Memory!! 3D array not allocated.\n");
 ...
 for (zz=0;zz < pstPar->DEpairs;zz++)
 {
  ...
  {
   (*pstRUN)->Table_JumpRate[zy][zz] = 2.38f / sqrt(2.0f * (float)(zz + 1) * (float)(zy + 1));
   }
  }
...
void GenCR(struct DREAM_Parameters **ppPar, float ***pppCR, float ****ptCR)
{
 ...
 for (zz=0;zz < (*ppPar)->nCR;zz++)
 {
  ...
  //Reshape CR 1D array into 2D array of "(*ppPar)->seq" rows and "(*ppPar)->steps" columns
  cont = 0;
...
void multrnd(int **X, int n, float ***p, int ncols, int m)
{
 ...
 for (i=0;i < m;i++)
 {
  for (in=0;in < n;in++)
  {
   ...
   r[in] = UNIFORM_RAND; //generate Uniformly distributed pseudorandom numbers [0 1)
   }
  //cumulative sum
  ...
  for (j=0;j < ncols;j++)
  {
   ...
   //Y - multinomial probabilities of the generated random deviates (optional output)
   //Y[j] = (float)((*X)[j]) / (float)(n); 
   }
  }
 }
...
void InitSequences(float **X, float ***Sequences, struct DREAM_Parameters *MCMCPar)
{
 int qq, kk;
...
void Gelman(float **R_Stat, int R_Stat_Index, float ***Sequences, int numSamples, int numParams, int numChains, int start_loc) {
...
if (numSamples < 10) {
	...
	for (iChains = 0; iChains < numChains; iChains++) { // For each sequence
		for (iParams = 0; iParams < numParams; iParams++) { // For each parameter
    			for (iSamples = 0; iSamples < numSamples - 1; iSamples++) { // For each sample
	...
	for (iParams = 0; iParams < numParams; iParams++) {
		sigma2[iParams] = ((numSamplesf - 1)/numSamplesf) * W[iParams] + (1/numSamplesf) * B[iParams];
	}
	...
	for (iParams = 0; iParams < numParams; iParams++) {
		if (sigma2[iParams] == 0) {
			R_Stat[R_Stat_Index][iParams + 1] = 1.0;
   			...
   			R_Stat[R_Stat_Index][iParams + 1] = sqrt((numChainsf + 1)/numChainsf * sigma2[iParams]/W[iParams] - (numSamplesf-1)/numChainsf/numSamplesf);
		}
	}
	//Deallocating Memory
	free(sigma2);
...
void GetLocation(float **x_old, float *p_old, float *log_p_old, float **X, struct DREAM_Parameters *MCMCPar)
{
...
void DEStrategy(int *DEversion, struct DREAM_Parameters *MCMCPar)
{
 ...
 MEMORYCHECK(p_pair,"ERROR at DEStrategy: Out of Memory!! p_pair not allocated.\n");
 p_pair[0] = 0;
...
void offde(float **x_new, float **x_old, float **X, float **CR, struct DREAM_Parameters *MCMC, float **Table_JumpRate, struct Model_Input *Input, const char *BHandling, float *R2, const char *DR) {
 ...
 int *DEversion = NULL, *permarray = NULL, tt[MCMC->seq-1][MCMC->seq], idx[MCMC->seq-1], ii[MCMC->seq], *rr;
 ...
 float rndnum, Jump_Rate, delta[MCMC->n], temp_delta[MCMC->n];
	...
	for (i = 0; i < MCMC->seq; i++) {
		for (j = 0; j < MCMC->n; j++) {
			eps[i][j] = 1e-6 * eps[i][j]; // * (Input->ParRangeMax[j] - Input->ParRangeMin[j]);
	...
	if (strcmp(DR,"No")== 0) {
		...
		MEMORYCHECK(DEversion,"ERROR at offde: Out of Memory!! DEversion not allocated.\n");
		DEStrategy(DEversion, MCMC);
  
		//Generate series of permutations of chains
		permarray = (int*)malloc((MCMC->seq-1)*sizeof(int));
		MEMORYCHECK(permarray,"ERROR at offde: Out of Memory!! permarray not allocated.\n");
		for (i=0;i < MCMC->seq;i++) {
		...
		for (qq=0;qq < MCMC->seq;qq++) {
			...
			for (i=0;i < MCMC->n;i++) {
				if (D[qq][i] > (1 - CR[qq][0])) {
					...
					p_i[NrDim] = i;
					NrDim++; //Determine the number of dimensions that are going to be updated
				}
			}
			...
			if (rndnum < 0.8) {
				...
				for (i=0;i < NrDim;i++) {
					delta_x[qq][p_i[i]] = (1 + noise_x[qq][p_i[i]]) * Jump_Rate * delta[p_i[i]];
					//printf("delta_x(%i, %i) %f %f %f %f\n", qq, p_i[i], delta_x[qq][p_i[i]], (1 + noise_x[qq][p_i[i]]), Jump_Rate, delta[p_i[i]]);
...
void ReflectBounds(float **x_new, struct Model_Input *Input, int nmbOfIndivs, int Dim)
{
 ...
 for (i=0;i < nmbOfIndivs;i++)
 {
  for (j=0;j < Dim;j++)
  {
   ...
   if (x_new[i][j] < Input->ParRangeMin[j])
   {
    x_new[i][j] = Input->ParRangeMin[j] + UNIFORM_RAND*(Input->ParRangeMax[j] - Input->ParRangeMin[j]);
    }
...
void metrop(float **newgen, float *alpha, float *accept, float **x, float **p_x, float *log_p_x, float **x_old, float *p_old, float *log_p_old, struct Model_Input *pointerInput, struct DREAM_Parameters *pointerMCMC, int option)
{
...
void CalcDelta(float *delta_tot, struct DREAM_Parameters *MCMC, float *delta_normX, struct DREAM_Variables *RUNvar, int gnum)
{
...
void AdaptpCR(struct DREAM_Variables *RUNvar, struct DREAM_Parameters *MCMC, float *delta_tot)
{
 ...
 {
  RUNvar->pCR[0][i] = (float)(MCMC->seq) * (delta_tot[i]/RUNvar->lCR[i]) / sumarray(&delta_tot[0], MCMC->nCR, 1);
}
 ...
 for (i=0;i < MCMC->nCR;i++)
 {
  ...
  INFO_LOGF("pCR %i %f %f %f %f", i, RUNvar->pCR[0][i], delta_tot[i], RUNvar->lCR[i], sumarray(&delta_tot[0], MCMC->nCR, 1));
  }
 deallocate2D(&CRvector,1);
 }

void RemOutLierChains(float **X, struct DREAM_Variables *RUNvar, struct DREAM_Parameters *MCMC, struct DREAM_Output *output)
{
 ...
 MEMORYCHECK(mean_hist_logp, "ERROR at RemOutLierChains: Out of Memory!! mean_hist_logp not allocated.\n")
 ...
 MEMORYCHECK(chain_id, "ERROR at RemOutLierChains: Out of Memory!! chain_id not allocated.\n");
 ...
 {
  printf("ERROR at RemOutLierChains: Grubbs_test option has not been implemented\n");
  exit(1);
  }
 
 if (strcmp(MCMC->outlierTest, "Mahal_test") == 0)
 {
  printf("ERROR at RemOutLierChains: Mahal_test option has not been implemented\n");
  exit(1);
  }
...
void GenParSet(float **ParSet, struct DREAM_Variables *RUNvar, int post_Sequences, struct DREAM_Parameters *MCMC, FILE *fid, float *bestParams)
{
