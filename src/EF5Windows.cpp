...
int CreateWindows(HINSTANCE hInstance) {
    ...
    hMainWindow = CreateWindowEx(WS_EX_APPWINDOW | WS_EX_OVERLAPPEDWINDOW, "EF5Class", "Ensemble Framework For Flash Flood Forecasting", WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT , CW_USEDEFAULT, NULL, NULL, hInstance, NULL);
	if (!hMainWindow) {
        UnregisterClass("EF5Class", hInstance);
        FreeLibrary(hREDLL);
        return 0;
    }
    hInfo = CreateWindowEx(0, RICHEDIT_CLASS, "", WS_CHILD | WS_VSCROLL | ES_AUTOVSCROLL | ES_MULTILINE | ES_NOHIDESEL | ES_READONLY, 20, 20, 290, 120, hMainWindow, NULL, hInstance, NULL);
    if (!hInfo) {
    ...
    hTime = CreateWindowEx(WS_EX_TRANSPARENT, "STATIC", "", WS_CHILD , 20, 20, 290, 120, hMainWindow, NULL, hInstance, NULL);
    if (!hTime) {
                DestroyWindow(hMainWindow);
        UnregisterClass("EF5Class", hInstance);
        FreeLibrary(hREDLL);
    ...
    //hSend = CreateWindowEx(0, RICHEDIT_CLASS, "", WS_CHILD  , 20, 180, 290, 30, hMainWindow, NULL, hInstance, NULL);
    //if (!hSend) {
//		return 0;
//	}
...
//	wpSend = (WNDPROC)(LONG_PTR)SetWindowLong(hSend, GWL_WNDPROC, (LONG)(LONG_PTR)SendSubclass);
...
static LRESULT CALLBACK MainWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        ...
        case WM_SIZE: {
            ...
            SetWindowPos(hInfo, NULL, 20, 20, mainw.right - 40, mainw.bottom - 40, SWP_NOACTIVATE | SWP_NOZORDER);
