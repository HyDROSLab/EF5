...
FloatGrid *ReadFloatTRMMRTGrid(char *file, FloatGrid *grid) {
  ...
  if (!grid) {
    ...
    if (!grid->data) {
      WARNING_LOGF("TRMMRT file %s too large (out of memory) with %li rows", file, grid->numRows);
      delete grid;
      gzclose(fileH);
      return NULL;
    }
    for (long i = 0; i < grid->numRows; i++) {
      ...
      if (!grid->data[i]) {
        WARNING_LOGF("TRMMRT file %s too large (out of memory) with %li colums", file, grid->numCols);
        delete grid;
        gzclose(fileH);
        return NULL;
      }
  ...
  for (long i = 0; i < grid->numRows; i++) {
    ...
    for (int j = 0; j < grid->numCols; j++) {
      ...
      unsigned short realData = (shortData[j] >> 8) | ((shortData[j] & 0xFF) << 8);
      float floatData = 0;
