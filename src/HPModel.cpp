...
bool HPModel::InitializeModel(std::vector<GridNode> *newNodes, std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
...
void HPModel::SaveStates(TimeVar *currentTime, char *statePath, GridWriterFull *gridWriter) {
}

bool HPModel::WaterBalance(float stepHours, std::vector<float> *precip, std::vector<float> *pet, std::vector<float> *fastFlow, std::vector<float> *slowFlow, std::vector<float> *soilMoisture) {
...
void HPModel::WaterBalanceInt(GridNode *node, HPGridNode *cNode, float stepHours, float precipIn, float petIn, float *fastFlow, float *slowFlow) {
...
void HPModel::InitializeParameters(std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
  ...
  size_t numNodes = nodes->size();
	...
	for (size_t i = 0; i < numNodes; i++) {
		...
		memcpy(cNode->params, (*paramSettings)[node->gauge], sizeof(float)*PARAM_HP_QTY);
