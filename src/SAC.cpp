...
bool SAC::InitializeModel(std::vector<GridNode> *newNodes, std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
...
void SAC::SaveStates(TimeVar *currentTime, char *statePath, GridWriterFull *gridWriter) {
  DatedName timeStr;
  timeStr.SetNameStr("YYYYMMDD_HHUU");
  timeStr.ProcessNameLoose(NULL);
...
bool SAC::WaterBalance(float stepHours, std::vector<float> *precip, std::vector<float> *pet, std::vector<float> *fastFlow, std::vector<float> *slowFlow, std::vector<float> *soilMoisture) {
    ...
    slowFlow->at(i) += (cNode->dischargeS / (stepHours * 3600.0f));
		soilMoisture->at(i) = 100.0 * (cNode->UZTWC + cNode->UZFWC) / (cNode->params[PARAM_SAC_UZTWM] + cNode->params[PARAM_SAC_UZFWM]);
		if (!std::isfinite(soilMoisture->at(i))) {
			soilMoisture->at(i) = 0;
		} 
		//soilMoisture->at(i) = (cNode->UZTWC + cNode->UZFWC) / (cNode->params[PARAM_SAC_UZTWM] + cNode->params[PARAM_SAC_UZFWM]);
		//discharge->at(i) = (cNode->dischargeF + cNode->dischargeS) * node->area * 0.277777777777778f; // Convert from mm/time to cms;
		//printf(" Q %f\n", discharge->at(i));
		//LocalRouteQF(node, cNode);
		//LocalRouteSF(node, cNode);
	}
  
	return true;
}

void SAC::WaterBalanceInt(GridNode *node, SACGridNode *cNode, float stepHours, float precipIn, float petIn) {
  
	/*float precip = 0.0f; //precipIn * stepHours; // precipIn is mm/hr, precip is mm
   float pet = 20.0f; //petIn * stepHours; // petIn in mm/hr, pet is mm
   float DT = 3.0f / 24.0f; //stepHours / 24.0f;*/
	
  
	float precip = precipIn * stepHours; //stepHours; //stepHours; // precipIn is mm/hr, precip is mm
	...
	if ((cNode->UZTWC / cNode->params[PARAM_SAC_UZTWM]) < (cNode->UZFWC / cNode->params[PARAM_SAC_UZFWM])) {
		// Upper zone free water ratio exceeds upper zone tension
		// water ratio, thus transfer free water to tension
		float UZRAT = (cNode->UZTWC + cNode->UZFWC) / (cNode->params[PARAM_SAC_UZTWM] + cNode->params[PARAM_SAC_UZFWM]);
	...
	E3 = RED * (cNode->LZTWC / (cNode->params[PARAM_SAC_UZTWM] + cNode->params[PARAM_SAC_LZTWM]));
	cNode->LZTWC -= E3;
	...
	float RATLZ = (cNode->LZTWC + cNode->LZFPC + cNode->LZFSC - cNode->params[PARAM_SAC_RSERV]) / (cNode->params[PARAM_SAC_LZTWM] + cNode->params[PARAM_SAC_LZFPM] + cNode->params[PARAM_SAC_LZFSM] - cNode->params[PARAM_SAC_RSERV]);
	...
	E5 = E1 + (RED + E2) * ((cNode->ADIMC - E1 - cNode->UZTWC) / (cNode->params[PARAM_SAC_UZTWM] + cNode->params[PARAM_SAC_LZTWM]));
	...
	if (TWX < 0.0f) {
		...
		cNode->UZTWC = cNode->params[PARAM_SAC_UZTWM]; // Moisture in excess of UZTW storage
	}
  
	cNode->ADIMC = cNode->ADIMC + precip - TWX;
  
	// Compute impervious area runoff
	float ROIMP = precip * cNode->params[PARAM_SAC_PCTIM]; // Runoff from minimum impervious area
  
	float SBF = 0.0f, SSUR = 0.0f, SIF = 0.0f, SPERC = 0.0f, SDRO = 0.0f, SPBF = 0.0f;
  
	float NINC = floor(1.0f + 0.2f * (cNode->UZFWC + TWX)); // Number of time increments that the time interval is divided into for further soil-moisture accounting.
	// No one increment will exceed 5.0 millimeters of UZFWC+PAV
	
	float DINC = (1.0f / NINC) * DT; // Length of each increment in days
  
	float PINC = TWX / NINC; // Amount of available moisture for each increment. Compute free water depletion fractions for the time increment
  // being used-basic depletions are for one day
	...
	for (float i = 0.0f; i < NINC; i = i + 1.0f) {
		float ADSUR = 0.0f;
		float RATIO = (cNode->ADIMC - cNode->UZTWC) / cNode->params[PARAM_SAC_LZTWM];
		if (RATIO < 0.0f) {
			RATIO = 0.0f;
		}
    
		float ADDRO = PINC * pow(RATIO, 2.0f); // Amount of direct runoff from the area ADIMP
       ...
       cNode->ADIMC = 0.0f;
			...
			if (cNode->ADIMC > (cNode->params[PARAM_SAC_UZTWM] + cNode->params[PARAM_SAC_LZTWM])) {
				ADDRO = ADDRO + cNode->ADIMC - (cNode->params[PARAM_SAC_UZTWM] + cNode->params[PARAM_SAC_LZTWM]);
				cNode->ADIMC = cNode->params[PARAM_SAC_UZTWM] + cNode->params[PARAM_SAC_LZTWM];
			}
			SDRO = SDRO + ADDRO * cNode->params[PARAM_SAC_ADIMP];
			if (cNode->ADIMC < 0.00001f) {
		...
		float DEFR = 1.0f - ((cNode->LZTWC + cNode->LZFPC + cNode->LZFSC) / (cNode->params[PARAM_SAC_LZTWM] + cNode->params[PARAM_SAC_LZFPM] + cNode->params[PARAM_SAC_LZFSM]));
		...
		PERC = PERC * (1.0f + cNode->params[PARAM_SAC_ZPERC] * pow(DEFR, cNode->params[PARAM_SAC_REXP])) * FR;
		...
		float CHECK = cNode->LZTWC + cNode->LZFPC + cNode->LZFSC + PERC - cNode->params[PARAM_SAC_LZTWM] - cNode->params[PARAM_SAC_LZFPM] - cNode->params[PARAM_SAC_LZFSM];
		if (CHECK > 0.0f) {
			PERC -= CHECK;
			cNode->UZFWC += CHECK;
		}
		...
		if (PERCF != 0.0f) {
			float HPL = cNode->params[PARAM_SAC_LZFPM] / (cNode->params[PARAM_SAC_LZFPM] + cNode->params[PARAM_SAC_LZFSM]);
			...
			float FRACP = (HPL * 2.0f * (1.0f - RATLP)) / ((1.0f - RATLP) + (1.0f - RATLS));
	...
	float BFCC = TBF * (1.0f / (1.0f + cNode->params[PARAM_SAC_SIDE])); // Baseflow, channel component
	...
	float E4 = (pet - EUSED) * cNode->params[PARAM_SAC_RIVA]; // / stepHours; // ET from Riparian vegetation
	...
	//printf(" %f %f %f %f %f\n", EUSED, E5, E4, pet, cNode->params[PARAM_SAC_RIVA]);
	//printf(" %f %f %f\n", SURF, GRND, TET);
	//printf("1: %f %f %f %f %f %f", cNode->UZTWC, cNode->UZFWC, cNode->LZTWC, cNode->LZFSC, cNode->LZFPC, cNode->ADIMC);
  
}

void SAC::InitializeParameters(std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
	...
	for (std::vector<GridNode>::iterator itr = nodes->begin(); itr != nodes->end(); itr++) {
		...
		memcpy(cNode->params, (*paramSettings)[node->gauge], sizeof(float)*PARAM_SAC_QTY);
    
		// Initialize states
		cNode->UZTWC = cNode->params[PARAM_SAC_UZTWC] * cNode->params[PARAM_SAC_UZTWM]; //0.0f;
		cNode->UZFWC = cNode->params[PARAM_SAC_UZFWC] * cNode->params[PARAM_SAC_UZFWM]; //0.0f; //2.773;
		cNode->LZTWC = cNode->params[PARAM_SAC_LZTWC] * cNode->params[PARAM_SAC_LZTWM]; //0.0f; //286.7;
		cNode->LZFSC = cNode->params[PARAM_SAC_LZFSC] * cNode->params[PARAM_SAC_LZFSM]; //0.0f;
		cNode->LZFPC = cNode->params[PARAM_SAC_LZFPC] * cNode->params[PARAM_SAC_LZFPM]; //0.0f; //154.2;
