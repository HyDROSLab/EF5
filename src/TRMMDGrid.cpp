...
FloatGrid *ReadFloatTRMMDGrid(char *file, FloatGrid *grid) {
  ...
  if (!grid) {
    ...
    if (!grid->data) {
      WARNING_LOGF("TRMM Daily file %s too large (out of memory) with %li rows", file, grid->numRows);
      delete grid;
      gzclose(fileH);
      return NULL;
    }
    for (long i = 0; i < grid->numRows; i++) {
      ...
      if (!grid->data[i]) {
        WARNING_LOGF("TRMM Daily file %s too large (out of memory) with %li cols", file, grid->numCols);
        delete grid;
        gzclose(fileH);
        return NULL;
      }
  ...
  if (shortData) {
    WARNING_LOGF("TRMM Daily file %s too large (out of memory) temporary storage", file);
    delete grid;
    gzclose(fileH);
    return NULL;
  }
  ...
  for (long i = 0; i < grid->numRows; i++) {
    ...
    for (int j = 0; j < grid->numCols; j++) {
      int realJ = (j > 720) ? (j - 720) : (j + 720); // Flip this about the Y-axis
