...
void VCInundation::ComputeLayers(size_t nodeIndex, GridNode *node, VCInundationGridNode *cNode) {
		...
		for (int i = 1; i < FLOW_QTY; i++) {
			...
			if (TestUpstream(currentN->x, currentN->y, (FLOW_DIR)i, &nextNode)) {
					...
					for (size_t nodeI = currentN->index; nodeI < nodes->size(); nodeI++) {
						if (nodes->at(nodeI).x == nextNode.x && nodes->at(nodeI).y == nextNode.y) {
	...
	//printf("Found %i upstream cells for node %i, FAM is %f\n", upstreamCount, (int)nodeIndex, g_FAM->data[node->y][node->x]);
	...
	for (int i = 0; i < upstreamCount; i++) {
		...
		float heightDiff = g_DEM->data[up->y][up->x] - g_DEM->data[current->y][current->x];
		if (heightDiff <= 0.01) {
			continue;
			//heightDiff = 0.01;
		}
...
bool VCInundation::Inundation(std::vector<float> *discharge, std::vector<float> *depth) {
...
void VCInundation::InitializeParameters(std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
  ...
  for (size_t i = 0; i < numNodes; i++) {
    ...
    memcpy(cNode->params, (*paramSettings)[node->gauge], sizeof(float)*PARAM_VCI_QTY);
