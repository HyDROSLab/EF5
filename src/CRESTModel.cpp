...
bool CRESTModel::InitializeModel(std::vector<GridNode> *newNodes, std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
...
void CRESTModel::SaveStates(TimeVar *currentTime, char *statePath, GridWriterFull *gridWriter) {
	DatedName timeStr;
  timeStr.SetNameStr("YYYYMMDD_HHUU");
  timeStr.ProcessNameLoose(NULL);
...
bool CRESTModel::WaterBalance(float stepHours, std::vector<float> *precip, std::vector<float> *pet, std::vector<float> *fastFlow, std::vector<float> *slowFlow, std::vector<float> *soilMoisture) {
  ...
  for (size_t i = 0; i < numNodes; i++) {
    GridNode *node = &nodes->at(i);
    CRESTGridNode *cNode = &(crestNodes[i]);
    WaterBalanceInt(node, cNode, stepHours, precip->at(i), pet->at(i), &(fastFlow->at(i)),  &(slowFlow->at(i)));
    soilMoisture->at(i) = cNode->states[STATE_CREST_SM] * 100.0 / cNode->params[PARAM_CREST_WM];
  }
  
	return true;
}

void CRESTModel::WaterBalanceInt(GridNode *node, CRESTGridNode *cNode, float stepHours, float precipIn, float petIn, float *fastFlow, float *slowFlow) {
  ...
  // We have more water coming in than leaving via ET.
	if (precip > adjPET) {
		double precipSoil = (precip - adjPET) * (1 - cNode->params[PARAM_CREST_IM]); // This is the precip that makes it to the soil
    double precipImperv = precip - adjPET - precipSoil; // Portion of precip on impervious area
    
    //cNode->states[STATE_CREST_SM] += *slowFlow;
    //*slowFlow = 0.0;
    
    double interflowExcess = cNode->states[STATE_CREST_SM] - cNode->params[PARAM_CREST_WM];
    ...
    if (cNode->states[STATE_CREST_SM] > cNode->params[PARAM_CREST_WM]) {
      cNode->states[STATE_CREST_SM] = cNode->params[PARAM_CREST_WM];
		...
		if (cNode->states[STATE_CREST_SM] < cNode->params[PARAM_CREST_WM]) {
			double Wmaxm = cNode->params[PARAM_CREST_WM] * (1 + cNode->params[PARAM_CREST_B]);
			double A = Wmaxm * (1 - pow(1 - cNode->states[STATE_CREST_SM] / cNode->params[PARAM_CREST_WM], 1 / (1 + cNode->params[PARAM_CREST_B])));
			if (precipSoil + A >= Wmaxm) {
				R = precipSoil - (cNode->params[PARAM_CREST_WM] - cNode->states[STATE_CREST_SM]); // Leftovers after filling SM
				
        
        
				if (R < 0) {
          printf("R to %f, [%f, %f, %f, %f, %f, %f]\n", R, cNode->params[PARAM_CREST_WM], cNode->params[PARAM_CREST_B], cNode->states[STATE_CREST_SM], A, Wmaxm, precipSoil);
          R = 0.0;
				}
        ...
        double infiltration = cNode->params[PARAM_CREST_WM] * (pow(1 - A / Wmaxm, 1 + cNode->params[PARAM_CREST_B])  - pow(1 - (A + precipSoil) / Wmaxm, 1 + cNode->params[PARAM_CREST_B]));
				if (infiltration > precipSoil) {
          infiltration = precipSoil;
        } else if (infiltration < 0.0) {
          printf("Infiltration went to %f, [%f, %f, %f, %f, %f, %f]\n", infiltration, cNode->params[PARAM_CREST_WM], cNode->params[PARAM_CREST_B], cNode->states[STATE_CREST_SM], A, Wmaxm, precipSoil);
        }
    ...
    // Now R is excess water, split it between overland & interflow
		...
		temX = (cNode->states[STATE_CREST_SM] + Wo) / cNode->params[PARAM_CREST_WM] / 2 * (cNode->params[PARAM_CREST_FC] * stepHours); // Calculate how much water can infiltrate
		...
		cNode->excess[CREST_LAYER_OVERLAND] = R - cNode->excess[CREST_LAYER_INTERFLOW] + precipImperv;
    
		cNode->actET = adjPET;
    
    cNode->excess[CREST_LAYER_INTERFLOW] += interflowExcess; // Extra interflow that got routed.
...
void CRESTModel::InitializeParameters(std::map<GaugeConfigSection *, float *> *paramSettings, std::vector<FloatGrid *> *paramGrids) {
  ...
  size_t numNodes = nodes->size();
	...
	for (size_t i = 0; i < numNodes; i++) {
		...
		/*if (i == 0) {
			...
			printf("Thread %i, %f\n", omp_get_thread_num(), paramsBlah[PARAM_CREST_QTY-1]);
		}*/
		// Copy all of the parameters over
		memcpy(cNode->params, (*paramSettings)[node->gauge], sizeof(float)*PARAM_CREST_QTY);
