...
bool TempReader::Read(char *file, SUPPORTED_TEMP_TYPES type, std::vector<GridNode> *nodes, std::vector<float> *currentTemp, std::vector<float> *prevTemp, bool hasF) {
	if (!strcmp(lastTempFile, file)) {
		...
		return true; // This is the same temp file that we read last time, we assume currentPET is still valid!
	}
  ...
  if (hasF) {
	...
	// We have two options now... Either the temp grid & the basic grids are the same
	...
	if (g_DEM->IsSpatialMatch(tempGrid)) {
		...
		GridLoc pt;
                for (size_t i = 0; i < nodes->size(); i++) {
                        GridNode *node = &(nodes->at(i));
                        if (tempGrid->GetGridLoc(node->refLoc.x, node->refLoc.y, &pt) && tempGrid->data[pt.y][pt.x] != tempGrid->noData) {
