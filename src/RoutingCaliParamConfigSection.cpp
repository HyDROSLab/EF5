...
std::map<std::string, RoutingCaliParamConfigSection *> g_routingCaliParamConfigs[ROUTE_QTY];
...
char *RoutingCaliParamConfigSection::GetName() {
	return name;
}

CONFIG_SEC_RET RoutingCaliParamConfigSection::ProcessKeyValue(char *name, char *value) {
	int numParams = numRouteParams[route];
...
bool RoutingCaliParamConfigSection::IsDuplicate(char *name, ROUTES routeVal) {
        std::map<std::string, RoutingCaliParamConfigSection *>::iterator itr = g_routingCaliParamConfigs[routeVal].find(name);
