...
void ARS::Initialize(CaliParamConfigSection *caliParamConfigNew, RoutingCaliParamConfigSection *routingCaliParamConfigNew, SnowCaliParamConfigSection *snowCaliParamConfigNew, int numParamsWBNew, int numParamsRNew, int numParamsSNew, Simulator *simNew) {
...
void ARS::CalibrateParams() {
	...
	while(goodSets < burnInSets || scoreDiff > convergenceCriteria) {
		...
		if (((goal == OBJECTIVE_GOAL_MAXIMIZE) && objScore < minObjScore) || ((goal == OBJECTIVE_GOAL_MINIMIZE) && objScore > minObjScore)) {
			continue;
		} else {
		...
		for (std::list<ARS_INFO *>::iterator itr = topSets.begin(); itr != topSets.end(); itr++) {
			...
			if (((goal == OBJECTIVE_GOAL_MAXIMIZE) && objScore > current->objScore) || ((goal == OBJECTIVE_GOAL_MINIMIZE) && objScore < current->objScore)) {
		...
		// Lets see if the top number of sets hasn't filled yet, if so add it to the back
		...
		if (insertedParams) {
			...
			printf("ConvC %f (%f, %f), total runs %i, good runs %i\n", (top->objScore - bottom->objScore), top->objScore, bottom->objScore, totalSets, goodSets);
		}
