...
Config::Config(const char *configName) {
	//Copy over the config name into the class variable and ensure it is null terminated
...
CONFIG_PARSE_RESULTS Config::ParseConfig() {
	...
	buffer[fileLen] = 0; buffer[fileLen + 1] = 0; buffer[fileLen + 2] = 0; buffer[fileLen + 3] = 0;
	...
	for (i = 0; i < fileLen; i++) {
		switch (state) {
			...
			case PARSE_SECTION_NAME: {
				if (buffer[i] == ' ') {
					...
					//Invalid case!
					ERROR_LOGF("%s(%i): Invalid section header contains newline", name, line);
					delete [] buffer;
					return CONFIG_INV_SECHEAD;
				} else {
					...
					if (nameIdx == CONFIG_MAX_LEN) {
						ERROR_LOGF("%s(%i): Section name exceeds max length of %i", name, line, CONFIG_MAX_LEN);
						delete [] buffer;
						return CONFIG_INV_SECHEAD;
					}
					continue;
        ...
        if (buffer[i] == ']') {
          ...
          currentSection = GetConfigSection(nameBuf, valBuf);
          if (currentSection == NULL) {
            ERROR_LOGF("%s(%i): Unknown or invalid section \"%s\"", name, line, nameBuf);
            delete [] buffer;
            return CONFIG_INV_SECHEAD;
          }
          continue;
        ...
        break;
			...
			case PARSE_NAME: {
				if (buffer[i] == '=') {
					...
					nameBuf[nameIdx] = 0; //Add null terminator
					ERROR_LOGF("%s(%i): Invalid key (%s) contains newline", name, line, nameBuf);
					delete [] buffer;
					...
					nameIdx++;
					if (nameIdx == CONFIG_MAX_LEN) {
            ERROR_LOGF("%s(%i): Key exceeds max length of %i", name, line, CONFIG_MAX_LEN);
            delete [] buffer;
            return CONFIG_INV_NAME;
          }
					continue;
			...
			case PARSE_VALUE: {
				if (buffer[i] == ' ' || buffer[i] == '\n' || buffer[i] == '\r') {
					...
					valIdx++;
					if (valIdx == CONFIG_MAX_LEN) {
            ERROR_LOGF("%s(%i): Value exceeds max length of %i", name, line, CONFIG_MAX_LEN);
            delete [] buffer;
            return CONFIG_INV_NAMEKEY;
          }
					continue;
...
ConfigSection *Config::GetConfigSection(char *sectionName, char *sectionVal) {
	...
	if (strcasecmp(sectionName, "basic") == 0) {
      ...
      return newSec;
		...
		g_precipConfigs.insert(std::pair<std::string, PrecipConfigSection *>(sectionVal, preSec));
		newSec = preSec;
    ...
    PETConfigSection *petSec = new PETConfigSection();
		g_petConfigs.insert(std::pair<std::string, PETConfigSection *>(sectionVal, petSec));
		newSec = petSec;
    ...
    g_tempConfigs.insert(std::pair<std::string, TempConfigSection *>(sectionVal, tempSec));
    newSec = tempSec;
    ...
    g_gaugeConfigs.insert(std::pair<std::string, GaugeConfigSection *>(sectionVal, gaugeSec));
    newSec = gaugeSec;
    ...
    g_basinConfigs.insert(std::pair<std::string, BasinConfigSection *>(sectionVal, basinSec));
    newSec = basinSec;
    ...
    g_taskConfigs.insert(std::pair<std::string, TaskConfigSection *>(sectionVal, taskSec));
    newSec = taskSec;
    ...
    g_ensTaskConfigs.insert(std::pair<std::string, EnsTaskConfigSection *>(sectionVal, taskSec));
    newSec = taskSec;
		...
		for (int i = 0; i < MODEL_QTY; i++) {
          ...
          return newSec;
				...
				ParamSetConfigSection *psSec = new ParamSetConfigSection(sectionVal, (MODELS)i);
				g_paramSetConfigs[i].insert(std::pair<std::string, ParamSetConfigSection *>(sectionVal, psSec));
				newSec = psSec;
				break;
        ...
        CaliParamConfigSection *cpSec = new CaliParamConfigSection(sectionVal, (MODELS)i);
        g_caliParamConfigs[i].insert(std::pair<std::string, CaliParamConfigSection *>(sectionVal, cpSec));
        newSec = cpSec;
        break;
			}
		}
    ...
    for (int i = 0; i < ROUTE_QTY; i++) {
          ...
          return newSec;
				...
				RoutingParamSetConfigSection *psSec = new RoutingParamSetConfigSection(sectionVal, (ROUTES)i);
				g_routingParamSetConfigs[i].insert(std::pair<std::string, RoutingParamSetConfigSection *>(sectionVal, psSec));
				newSec = psSec;
				break;
        ...
        RoutingCaliParamConfigSection *cpSec = new RoutingCaliParamConfigSection(sectionVal, (ROUTES)i);
        g_routingCaliParamConfigs[i].insert(std::pair<std::string, RoutingCaliParamConfigSection *>(sectionVal, cpSec));
        newSec = cpSec;
        break;
			}
		}
    ...
    for (int i = 0; i < SNOW_QTY; i++) {
      if (strcasecmp(sectionName, snowParamSetStrings[i]) == 0) {
        ...
        SnowParamSetConfigSection *psSec = new SnowParamSetConfigSection(sectionVal, (SNOWS)i);
        g_snowParamSetConfigs[i].insert(std::pair<std::string, SnowParamSetConfigSection *>(sectionVal, psSec));
        newSec = psSec;
        break;
        ...
        SnowCaliParamConfigSection *cpSec = new SnowCaliParamConfigSection(sectionVal, (SNOWS)i);
        g_snowCaliParamConfigs[i].insert(std::pair<std::string, SnowCaliParamConfigSection *>(sectionVal, cpSec));
        newSec = cpSec;
        break;
      }
    }
    ...
    for (int i = 0; i < INUNDATION_QTY; i++) {
      if (strcasecmp(sectionName, inundationParamSetStrings[i]) == 0) {
        if (InundationParamSetConfigSection::IsDuplicate(sectionVal, (INUNDATIONS)i)) {
          ERROR_LOGF("Duplicate Inundation ParamSet section \"%s\"!", sectionVal);
          return newSec;
        }
        InundationParamSetConfigSection *psSec = new InundationParamSetConfigSection(sectionVal, (INUNDATIONS)i);
        g_inundationParamSetConfigs[i].insert(std::pair<std::string, InundationParamSetConfigSection *>(sectionVal, psSec));
        newSec = psSec;
        break;
      } else if (strcasecmp(sectionName, inundationCaliParamStrings[i]) == 0) {
        if (InundationCaliParamConfigSection::IsDuplicate(sectionVal, (INUNDATIONS)i)) {
          ERROR_LOGF("Duplicate Inundation CaliParam section \"%s\"!", sectionVal);
          return newSec;
        }
        InundationCaliParamConfigSection *cpSec = new InundationCaliParamConfigSection(sectionVal, (INUNDATIONS)i);
        g_inundationCaliParamConfigs[i].insert(std::pair<std::string, InundationCaliParamConfigSection *>(sectionVal, cpSec));
        newSec = cpSec;
        break;
      }
    }	
