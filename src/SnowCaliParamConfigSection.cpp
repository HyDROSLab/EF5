...
std::map<std::string, SnowCaliParamConfigSection *> g_snowCaliParamConfigs[SNOW_QTY];
...
char *SnowCaliParamConfigSection::GetName() {
	return name;
}

CONFIG_SEC_RET SnowCaliParamConfigSection::ProcessKeyValue(char *name, char *value) {
	int numParams = numSnowParams[snow];
...
CONFIG_SEC_RET SnowCaliParamConfigSection::ValidateSection() {
	...
	for (int i = 0; i < numParams; i++) {
		if (!paramsSet[i]) {
			ERROR_LOGF("Incomplete parameter set! Parameter \"%s\" was not given a value.", snowParamStrings[snow][i]);
			return INVALID_RESULT;
		}
	}
...
bool SnowCaliParamConfigSection::IsDuplicate(char *name, SNOWS snowVal) {
        std::map<std::string, SnowCaliParamConfigSection *>::iterator itr = g_snowCaliParamConfigs[snowVal].find(name);
